...
while true; do
  ...
  response=$(curl -s "$API_URL" -H "Content-Type: application/json" -d "{\"contents\":{\"role\":\"user\",\"parts\": [{\"text\":\"$question\"}]}}")

  text=$(echo "$response" | jq -r '.candidates[0].content.parts[0].text')
  echo -e "====================\n"

  echo "$text" # TODO: handle if the response is empty
  echo -e "\n"
done
